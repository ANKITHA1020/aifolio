...
export default function ClassicTemplate({ portfolio, templateConfig }: ClassicTemplateProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const colors = getTemplateColors("classic", templateConfig);
  ...
  return (
    <div ref={containerRef} className="template-classic template-container">
      {headerComponent && (
        <TemplateHeader
      ...
      {projectsComponent?.content.images && Array.isArray(projectsComponent.content.images) && projectsComponent.content.images.length > 0 && (
        <section className="template-classic-section">

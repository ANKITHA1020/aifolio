...
export default function ModernTemplate({ portfolio, templateConfig }: ModernTemplateProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const colors = getTemplateColors("modern", templateConfig);
  ...
  return (
    <div ref={containerRef} className="template-modern template-container">
      {headerComponent && (
        <TemplateHeader
      ...
      {projectsComponent?.content.images && Array.isArray(projectsComponent.content.images) && projectsComponent.content.images.length > 0 && (
        <section className="template-modern-section">

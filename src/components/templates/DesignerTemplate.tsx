...
export default function DesignerTemplate({ portfolio, templateConfig }: DesignerTemplateProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const colors = getTemplateColors("designer", templateConfig);
  ...
  return (
    <div ref={containerRef} className="template-designer template-container">
      {headerComponent && (
        <TemplateHeader
      ...
      {projectsComponent?.content.images && Array.isArray(projectsComponent.content.images) && projectsComponent.content.images.length > 0 && (
        <section className="template-designer-section">
          <div className="max-w-6xl mx-auto">
            <h2 className="template-section-title">Portfolio Gallery</h2>
            <ImageGallery 
              images={projectsComponent.content.images.filter((img: any) => img && img.url)} 
              layout="masonry" 
            />
          </div>
        </section>
